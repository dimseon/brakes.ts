...
import {defaultOptionsInterface} from './Brakes'
import { Bucket } from './Bucket'; 


/* Example Default Options */
const defaultStatOptions: defaultOptionsInterface = {
  bucketSpan: 1000,
  bucketNum: 60,
...
export class Stats extends EventEmitter {
  constructor(opts: defaultOptionsInterface) {
    super();
    this['_opts'] = Object.assign({}, defaultStatOptions, opts);
  ...
  */
  _generateStats(buckets: Bucket[], includeLatencyStats?: boolean | undefined) {
    // reduce buckets
    ...
    if (includeLatencyStats) {
      ...
      this['_opts'].percentiles.forEach((p:number) => {
  ...
  */
  ...
  _shiftAndPush(arr: Bucket[], item: Bucket) {
    arr.push(item);
    arr.shift();
    return arr;
  }
